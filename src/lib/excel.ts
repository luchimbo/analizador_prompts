...
export function buildImprovementComparisonExcelBuffer(rows: ImprovementComparisonRow[]): Buffer {
  ...
  const summaryRows = rows.map((row) => ({
    ...
    Product_Points_Before: row.productPointsBefore ?? "",
    Product_Points_After: row.productPointsAfter ?? "",
    Product_Points_Difference: row.productPointsDifference ?? "",
    Vendor_Points_Before: row.vendorPointsBefore ?? "",
    Vendor_Points_After: row.vendorPointsAfter ?? "",
    Vendor_Points_Difference: row.vendorPointsDifference ?? "",
    URL_Points_Before: row.urlPointsBefore ?? "",
    URL_Points_After: row.urlPointsAfter ?? "",
    URL_Points_Difference: row.urlPointsDifference ?? "",
    Rank_Points_Before: row.rankPointsBefore ?? "",
    Rank_Points_After: row.rankPointsAfter ?? "",
    Rank_Points_Difference: row.rankPointsDifference ?? "",
    Internal_Bonus_Before: row.internalBonusBefore ?? "",
    Internal_Bonus_After: row.internalBonusAfter ?? "",
    Internal_Bonus_Difference: row.internalBonusDifference ?? "",
    External_Penalty_Before: row.externalPenaltyBefore ?? "",
    External_Penalty_After: row.externalPenaltyAfter ?? "",
    External_Penalty_Difference: row.externalPenaltyDifference ?? "",
  }));
